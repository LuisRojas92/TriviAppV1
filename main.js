...
function imprimirPreguntas(arreglo) {
  ...
  const cadena = preguntas.map((pregunta) => {
    ...
    let cadena2 = `<div class='preguntaContenedor' data='${contador}'>
            <div class='pregunta'>
...
function colorearPregunta(elemento, cadena) {
  cadena == "correcto"
    ? (elemento.style.backgroundColor = "#D8EFC9")
    : (elemento.style.backgroundColor = "#F8C4B5");
}
...
function calificar() {
  if (numeroPreguntas == 10) {
    ...
    calificacionAviso.innerHTML =
      "Su calificación es de " + calificacion + "/" + numeroPreguntas;
